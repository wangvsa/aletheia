...
class SedovDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, samples_count, target):
        self.data_dir = data_dir
        self.samples_count = samples_count
        self.target = target
        print data_dir, samples_count

    def __getitem__(self, index):
        filename = self.data_dir + "/" + str(index) + ".npy"
        return np.load(filename), self.target

    def __len__(self):
        return self.samples_count

    # Cache a data file
    def get_data(self, filename, index, target):
        '''
        if filename == self.opend_filename:
            pass
        else :
            del self.opend_data
            self.opend_filename == filename
            self.opend_data = np.load(filename)
        return self.opend_data[index], target
        '''
        d = np.load(filename)
        sample = d[index].copy()
        del d
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(len(variables), 46, 3, stride=2),
            nn.BatchNorm2d(46),
            nn.ReLU(),
            #nn.Dropout(p = 0.2),
...
def get_concat_dataset():
    base_dir = "/u/sciteam/wang23/sources/train/"
    clean_data_files = ["data_dir_10", "data_dir_11","data_dir_12", "data_dir_13", "data_dir_14", "data_dir_15", "data_dir_16"]
    error_data_files = ["data_dir_10", "data_dir_11","data_dir_12", "data_dir_13", "data_dir_14", "data_dir_15"]

    datasets = []
    for f in clean_data_files:
        dir_path = base_dir + "clean/" + f + "/"
        datasets.append(SedovDataset(dir_path, 24321, 0.))
    for f in error_data_files:
        dir_path = base_dir + "error/" + f + "/"
        datasets.append(SedovDataset(dir_path, 30250, 1.))
